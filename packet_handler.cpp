...
pcap_t *server_pcap_t, *client_pcat_t;
...
std::vector<std::int> client_goodput_manage_vector；
std::vector<std::int> server_loss_manage_vector;
std::vector<std::int> server_rtt_manage_vector;
static double min_rtt_in_flow = MAX_FLOAT_NUM;

inline static double find_min_rtt(){
	for (auto tmp_rtt in server_rtt_manage_vector){
...
static void* send_sif(void* ptr){
	...
	while(true) {
		...
		unsigned packet_size= packet_header.len;
...
static bool cif_has_room_for(Mypacket * packet_tbs){
	double cur_packet_ts = (packet_tbs->packet_header->ts);
	u_int packet_size = packet_tbs->packet_header.len;
...
bool server_packet_handler(const struct pcap_pkthdr* packet_header, const u_char* packet_content){
	...
	if(!is_server_ip(nip2a(ip->sourceIP))) {
		cerr << "in server_handler capture a packet with its src IP: "
	...
	add_sif_queue(pkthdr, packet);
	return false;
}

/// deal with ACK packets
bool client_packet_handler(const struct pcap_pkthdr* pkthdr, const u_char* packet){
	// (const time_t*)&packet_header->ts.tv_sec;
	MyEthHdr* ethernet=(MyEthHdr *)packet_content;
	...
	// check if the packet is sent by us
	if(is_server_ip(nip2a(ip->sourceIP))) {
		cerr << "in client handler capture a packet with its src IP: "
			 << nip2a(ip->sourceIP) << std::endl;
		return;
	}
...
static bool add_sif_queue(const struct pcap_pkthdr* packet_header, const u_char* packet){
	...
	memcpy(packet_header,p_packet->pcap_pkthdr,sizeof(struct MyPacket))
	std::unique_lock<std::mutex> lock(sif_queue_mutex);
	sif_queueing_packet.push_back(p_packet);
	std::unique_lock<std::mutex> unlock(sif_queue_mutex);
}
...
static MyPacket* pop_sif_queue(){
	...
	std::unique_lock<std::mutex> lock(sif_queue_mutex);
	return_val = sif_queueing_packet.pop_front();
	std::unique_lock<std::mutex> unlock(sif_queue_mutex);
	return return_value;
}
...
static MyPacket* pop_cif_queue(){
	MyPacket * return_value;
	std::unique_lock<std::mutex> lock(cif_queue_mutex);
	return_val = cif_queueing_packet.pop_front();
	std::unique_lock<std::mutex> unlock(cif_queue_mutex);
	return return_value;
}
...
static bool update_state(MyIpHdr* ip_header, MyTcpHdr* tcp_header, double cur_time){
	...
	case FlowState::Waiting:
		if (is_server_ip(ip_header->destIP) && tcp_header->syn != 0) {
			realtime_flow_state = FlowState::Syn;
		} else {
			cerr << "unexpected packet at state Waiting" << std::endl;
		}
		break;
	case FlowState::Syn:
		if (is_server_ip(ip_header->sourceIP)) {
			if (tcp_header->syn != 0 && tcp_header->ack != 0) {
				...
				cerr << "unexpected server packet at state Syn" << std::endl;
			}
		} else {
			cerr << "unexpected client packet at state Syn" << std::endl;
		}
		break;
	case FlowState::SynAck:
		if (is_server_ip(ip_header->destIP)) {
			realtime_flow_state = FlowState::Flow;
		} else {
			cerr << "unexpected State SynAck";
		}
		break;
	case FlowState::Rst:
		cerr << "unexpected State Rst";
		realtime_flow_state = FlowState::Waiting;
		break;
	case FlowState::Flow:
		if (tcp_header->fin != 0) {
